      ...
      <tr>
        ...
        <td>
          <div class="text-center be-checkbox be-checkbox-sm">
            <input  type="checkbox"
                    ...
                    id="1{{Hashids::encode(substr($item->id, -12))}}"
                    @if ($item->ver == 1) checked @endif
            >
            <label  for="1{{Hashids::encode(substr($item->id, -12))}}"
                    data-atr = "ver"
                    class = "checkbox"                    
        ...
        <td>
          <div class="text-center be-checkbox be-checkbox-sm">
            <input  type="checkbox"
                    class="{{Hashids::encode(substr($item->id, -12))}}"
                    id="2{{Hashids::encode(substr($item->id, -12))}}"
                    @if ($item->anadir == 1) checked @endif
            >
            <label  for="2{{Hashids::encode(substr($item->id, -12))}}"
                    data-atr = "anadir"
                    class = "checkbox"                   
                    name="{{Hashids::encode(substr($item->id, -12))}}"
              ></label>
        ...
        <td>
          <div class="text-center be-checkbox be-checkbox-sm">
            <input  type="checkbox"
                    class="{{Hashids::encode(substr($item->id, -12))}}"
                    id="3{{Hashids::encode(substr($item->id, -12))}}"
                    @if ($item->modificar == 1) checked @endif
            >
            <label  for="3{{Hashids::encode(substr($item->id, -12))}}"
                    data-atr = "modificar"
                    class = "checkbox"                    
                    name="{{Hashids::encode(substr($item->id, -12))}}"
        ...
        <td>
          <div class="text-center be-checkbox be-checkbox-sm">
            <input  type="checkbox"
                    class="{{Hashids::encode(substr($item->id, -12))}}"
                    id="4{{Hashids::encode(substr($item->id, -12))}}"
                    @if ($item->todas == 1) checked @endif
            >
            <label  for="4{{Hashids::encode(substr($item->id, -12))}}"
                    data-atr = "todas"
                    class = "checkbox"                      
                    name="{{Hashids::encode(substr($item->id, -12))}}"
              ></label>
